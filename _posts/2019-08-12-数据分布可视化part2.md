...
sns.jointplot(x = df['a'],y = df['b'],
             ...
             kind = 'scatter',  #类型有'reg','resid','kde','hex','scatter'
             space = 0.3,
             size = 8,
             ratio = 5,
             marginal_kws = dict(bins = 15,rug = True)  #类型为kde时不能设置marginal_kws
             )
```
...
sns.stripplot(x = 'day',           #设置分组统计字段
             y = 'total_bill',     #数据分布统计字段
             data = tips,
             jitter = True,        #当点数据重合较多时,用该参数调整,可以设置间距如：jitter = 0.1 
...
sns.violinplot(x = 'day',y = 'total_bill',data = tips,
              ...
              palette = 'hls',            #调色盘
              order = ['Thur','Fri','Sat','Sun'],
              scale = 'area', #小提琴图的宽度：area-面积相同,count-样本数量决定宽度,width-宽度一样
              gridsize = 50,  #设置小提琴图边线的平滑度,值越大越平滑
