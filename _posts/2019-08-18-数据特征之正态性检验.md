...
s_r = s.reset_index(drop = False)			#重新排序
s_r['p'] = (s_r.index - 0.5) / len(s_r)
s_r['q'] = (s_r['value'] - mean) / std		#每个值标准化后的结果
#计算百分位数p(i)
#计算q值
...
>
> D：F0(x) 与 Fn(x) 差值的绝对值最大值		即 D= max | Fn(x) - F0(x) |
>
...
#.kstest方法：ks检验，参数分别是：待检验数据，检验方法，均值与标准差
...
以上是数据特征分析中的正态分布与正态性检验的全部内容，如有错误，恳请执政，感谢阅读～