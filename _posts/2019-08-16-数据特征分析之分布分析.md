...
![fbfx-2](https://raw.githubusercontent.com/spiderwu/spiderwu.github.io/master/img/tezhengfx-1/fbfx-2.png)
...
r_zj['频率'] = r_zj['参考总价频数'] / r_zj['参考总价频数'].sum()			#计算频率
r_zj['累计频率'] = r_zj['频率'].cumsum()								 #计算累计频率
r_zj['百分比'] = r_zj['频率'].apply(lambda x:'%.2f%%' % (x*100))		 #以百分比显示频率
r_zj['累计频率百分比'] = r_zj['累计频率'].apply(lambda x:'%.2f%%' % (x*100))  #以百分比显示累计频率
r_zj.style.bar(subset = ['频率','累计频率'])		#表格显示条形图
```

![fbfx-3](https://raw.githubusercontent.com/spiderwu/spiderwu.github.io/master/img/tezhengfx-1/fbfx-3.png)
